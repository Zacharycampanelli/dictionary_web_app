...
const Header = () => {
  ...
  useEffect(() => {
    ...
    isSmallDevice
      ? setLogoDimensions({ width: 34, height: 38 })
      : setLogoDimensions({ width: 38, height: 42 });
  }, [isSmallDevice]);

  // Render the header component with logo, font picker, and toggle switch
  return (
    <div className="flex justify-between w-full m-0 mb-12 md:pt-2">
      <div className="logo_container">
        <Logo width={logoDimensions.width} height={logoDimensions.height} />
      </div>
      <div className="flex justify-around w-2/3 options_container md:w-1/3">
        <FontPicker />
